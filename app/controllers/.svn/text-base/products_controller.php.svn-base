...
class ProductsController extends AppController {
	...
	function admin_index() {
		...
		if (!isset($_SESSION['admin_product_search']))
			$_SESSION['admin_product_search'] = ' ';

		if (isset($this->params['named']['search'])) {
			$_SESSION['admin_product_search'] = $this->params['named']['search'];
		} else {
			if (!isset($this->params['named']['page']))
				$_SESSION['admin_product_search'] = ' ';
		}
		
		$conditions = array('1' => "1 AND nome REGEXP '".addslashes($_SESSION['admin_product_search'])."*'");
	...
	function admin_add() {
		...
		$productStatuses = $this->_resolveForeignKey($this->Product, "ProductStatus", array('id', 'descrizione'), 'descrizione ASC');
		$suppliers = $this->_resolveForeignKey($this->Product, "Supplier", array('id', 'ragione_sociale'), 'ragione_sociale ASC');
	...
	function admin_edit($id = null) {
		...
		$productStatuses = $this->_resolveForeignKey($this->Product, "ProductStatus", array('id', 'descrizione'), 'descrizione ASC');
		$suppliers = $this->_resolveForeignKey($this->Product, "Supplier", array('id', 'ragione_sociale'), 'ragione_sociale ASC');
		$detailTypes = $this->_resolveForeignKey($this->Product->ProductTechnicalDetail, 'DetailType', array('id', 'descrizione'), 'descrizione ASC');
		...
		if (!empty($this->data)) {
			...
			if (isset($this->data['Product'])) {
				...
				if (!$deleteFotoDefault)
					$prodotto['Product']['foto_default'] = $this->data['Product']['foto_default'];
				else
					$prodotto['Product']['foto_default'] = '';

				$this->Product->save($prodotto);
			}
		}
		
		$this->Session->setFlash("Galleria foto modificata con successo", 'default', array('class' => 'flashsuccess'));
	...
	//mostra i prodotti  per categoria passando il valore $id
	function index_categorized($id = null) { 
		if (!$id) {
			$this->Session->setFlash("Prodotto non trovato");
			$this->redirect(array('action'=>'index'));
		} else { 
	...
	function makepdf() {
		$this->layout = 'empty';
		$products = $this->Product->query('SELECT * from products AS Product');
	...
	function find(){
		...
		$nome=$_GET['nome'];
		$products=$this->Product->query("SELECT * FROM products AS Product WHERE nome REGEXP '".addslashes($nome)."*'");
		$this->set('products',$products);
	...
	function beforeFilter() {
	    parent::beforeFilter(); 
	    $this->Auth->allowedActions = array('index', 'admin_upload_file', 'admin_find', 'makepdf');
	}
}
?>
